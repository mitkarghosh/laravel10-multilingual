...
function generateUniqueId() {
    ...
    return $unique  = 'MITS' . $timeNow . $randNumber;
}
...
/*
    * Function Name : stripeCustomerAccountCreation
    * Purpose       : This function is to create customer account on stripe
    * Author        :
    * Created Date  :
    * Modified date :
    * Input Params  : 
    * Return Value  : $fullName, $email, $phone
*/
function stripeCustomerAccountCreation($fullName, $email, $phone) {
    try {
        ...
        $response   = $stripe->customers->create([
            ...
            'email'         => $email,
            'phone'         => $phone,
            'description'   => 'Customer who will pay for Entrepreneur/Event',
        ]);
...
/*
    * Function Name : stripeAccountCreationUpdation
    * Purpose       : This function is to insert / update customer account stripe id
    * Author        :
    * Created Date  :
    * Modified date :
    * Input Params  : 
...
*/
function stripeAccountCreationUpdation($userData) {
    if ($userData->userDetailData != null) {
        if ($userData->userDetailData->stripe_customer_id == null) {
            $stripeCustomerId = stripeCustomerAccountCreation($userData->full_name, $userData->email, $userData->phone_no);

            if ($stripeCustomerId) {
                // Updating data into user_details table
                UserDetail::where(['user_id' => $userData->id])->update(['stripe_customer_id' => $stripeCustomerId]);
